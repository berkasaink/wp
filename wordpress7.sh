...
APACHE2=$(dpkg-query -W -f='${Status}' apache2 2>/dev/null | grep -c "ok installed")
  ...
  then
    ...
    apt-get install apache2 php7 --yes;
...
PHP7CURL=$(dpkg-query -W -f='${Status}' php7-curl 2>/dev/null | grep -c "ok installed")
  if [ $(dpkg-query -W -f='${Status}' php7-curl 2>/dev/null | grep -c "ok installed") -eq 0 ];
  then
    echo -e "${YELLOW}Installing php7-curl${NC}"
    apt-get install php7-curl --yes;
    elif [ $(dpkg-query -W -f='${Status}' php7-curl 2>/dev/null | grep -c "ok installed") -eq 1 ];
    then
      echo -e "${GREEN}php7-curl is installed!${NC}"
  fi
...
case $response in
  ...
  echo -e "${GREEN}Please, choose WordPress language you need (set ID or ENG): "
  read wordpress_lang

  if [ "$wordpress_lang" == 'ID' ];
    then
    wget https://id.wordpress.org/latest-id_ID.zip -O /tmp/$wordpress_lang.zip
  else
